...
function ModalCard({}) {
  ...
  return (
    <header className={`z-30 fixed top-0 right-0 left-0 bottom-0 ${theme? 'bg-black': 'bg-white'} flex font-semibold text-4xl justify-center ${isShowModal ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
      ...
      <Link target='_blank' to="https://api.whatsapp.com/send?phone=526531007177"><i className='bx bxl-whatsapp justify-self-start text-green-500 text-[30px] hover:opacity-50' ></i></Link>
      </div>
      </nav>
