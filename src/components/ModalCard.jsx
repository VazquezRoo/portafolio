...
function ModalCard({}) {
  ...
  return (
    <header className={`z-30 fixed top-0 right-0 left-0 bottom-0 ${theme? 'bg-black': 'bg-white'} flex font-semibold text-4xl justify-center ${isShowModal ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
      ...
      <Link target='_blank' to={'https://www.linkedin.com/in/cristian-vazquez-roo/'} className="w-[30px] aspect-square justify-self-end"><img src="/images/skills/github.png" alt="" className="hover:opacity-50" /></Link>
      <Link target='_blank' className="w-[30px] aspect-square justify-self-center"><img src="/images/Linkedin.png" alt="" className="hover:opacity-50 " /></Link>
      <Link to="https://api.whatsapp.com/send?phone=6531007177"><i className='bx bxl-whatsapp justify-self-start text-green-500 text-[30px] hover:opacity-50' ></i></Link>
      </div>
      </nav>
